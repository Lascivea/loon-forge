...
function convertProxyGroup(g) {
  ...
  const members = Array.isArray(g.proxies) ? g.proxies.join(',') : ''
  const url = esc(g.url || 'http://www.gstatic.com/generate_204')
  const interval = g.interval || 600
  const uses = Array.isArray(g.use) ? g.use : []
...
function convertRule(fields, ruleProviders) {
  ...
  if (type === 'RULE-SET') {
    ...
    const url = encodeURI(provider.url)
    const tag = esc(providerName)
    return { __remoteRule: true, line: `${url}, policy=${policy}, tag=${tag}, enabled=true` }
  }
  return null
}
...
function chainNameFromLine(line) {
  const idx = line.indexOf('=')
  return idx > 0 ? line.slice(0, idx).trim() : null
}

function mergeChains(autoChains, customChains) {
  const map = new Map()
  // 自定义 chain 作为补充，不覆盖自动生成的 dialer-proxy chain
  for (const line of customChains) {
    const name = chainNameFromLine(line)
    if (name) map.set(name, line)
  }
  // 自动生成优先
  for (const line of autoChains) {
    const name = chainNameFromLine(line)
    if (name) map.set(name, line)
  }
...
function buildLoonConfig(opts = {}) {
  const proxies = Array.isArray(opts.proxies) ? opts.proxies : []
  const groups = Array.isArray(opts.groups) ? opts.groups : []
  const rules = Array.isArray(opts.rules) ? opts.rules : []
  const ruleProviders = (opts.ruleProviders && typeof opts.ruleProviders === 'object') ? opts.ruleProviders : {}
  const pluginLines = Array.isArray(opts.pluginLines) ? opts.pluginLines : []
  const general = (opts.general && typeof opts.general === 'object') ? opts.general : {}
  const remoteProxies = Array.isArray(opts.remoteProxies) ? opts.remoteProxies : []
  const proxyChains = Array.isArray(opts.proxyChains) ? opts.proxyChains : []
  const remoteRules = Array.isArray(opts.remoteRules) ? opts.remoteRules : []
  const hosts = Array.isArray(opts.hosts) ? opts.hosts : []
  const rewrites = Array.isArray(opts.rewrites) ? opts.rewrites : []
  const scripts = Array.isArray(opts.scripts) ? opts.scripts : []
  const mitm = (opts.mitm && typeof opts.mitm === 'object') ? opts.mitm : {}
  const advancedTemplates = (opts.advancedTemplates && typeof opts.advancedTemplates === 'object') ? opts.advancedTemplates : {}
  ...
  // 4. 转换规则
  const ruleLines = []
  const autoRemoteRuleLines = []
  for (const r of rules) {
    const fields = String(r).split(',').map((s) => s.trim())
    const line = convertRule(fields, ruleProviders)
    if (!line) {
      skipped.rules.push(r)
    } else if (line && typeof line === 'object' && line.__remoteRule) {
      autoRemoteRuleLines.push(line.line)
    } else {
      ruleLines.push(line)
    }
  }
  ...
  // 自动 dialer-proxy chain 与自定义 chain 去重：自定义优先
  const proxyChainLines = mergeChains(autoProxyChains, customProxyChainLines)
  const customRemoteRuleLines = advancedTemplates.remoteRule
    ? sectionFromRaw(advancedTemplates.remoteRule)
    : renderRemoteRules(remoteRules)
  // 自动转换的 RULE-SET 放在 [Remote Rule] section 前面，补充的远程规则放后面
  const remoteRuleLines = [...autoRemoteRuleLines, ...customRemoteRuleLines]
